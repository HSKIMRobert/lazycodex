...
export function criterionId(index) {
    return `C${String(index + 1).padStart(3, "0")}`;
}
export function criteriaFromInput(input) {
    if (input.length === 0)
        invalid("provide at least one criterion or omit the field for placeholders.", { count: 0 });
    return input.map((entry, index) => {
        ...
        if (!isUserModel(userModel))
            return invalid(`entry ${index + 1} userModel must be one of ${ULW_LOOP_SUCCESS_CRITERION_USER_MODELS.join(", ")}.`, {
                index,
