...
// components/git-bash/src/codex-hook.ts
...
function parsePostCompactPayload(raw) {
  if (raw.trim().length === 0)
    return null;
  try {
    const parsed = JSON.parse(raw);
...
function applyGitBashPreToolUseReminder(payload, options = {}) {
...
function applyGitBashPostCompactReset(payload, options = {}) {
  if (payload.hook_event_name !== "PostCompact")
    return "";
  rmSync(reminderMarkerPath(payload.session_id, options.pluginDataRoot), { force: true });
  return "";
}
async function runGitBashHookCli(stdin, stdout, eventName = "pre-tool-use", options = {}) {
  try {
...
function isPostCompactPayload(value) {
  if (!isRecord(value))
    return false;
  return value["hook_event_name"] === "PostCompact" && typeof value["session_id"] === "string" && (value["transcript_path"] === undefined || value["transcript_path"] === null || typeof value["transcript_path"] === "string") && (value["trigger"] === undefined || typeof value["trigger"] === "string");
}
...
// components/git-bash/src/cli.ts
